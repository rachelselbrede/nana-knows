...
const blue = { name: "the blue cardigan", units: "in", craft: "knit", easeIdx: 2, patternGauge: "18", patternRowGauge: "24", sizesText: "32, 36, 40", yardsText: "900, 1000, 1100", bust: "38", myGauge: "21", myRowGauge: "26", perSkein: "220", skeins: "6" };
const green = { name: "the green blanket", units: "cm", craft: "crochet", easeIdx: 3, sizesText: "100, 120", bust: "96", myGauge: "", myRowGauge: "", perSkein: "", skeins: "" };
...
describe("readNotebook: nothing on trust", () => {
  ...
  test("a notebook of pages keeps their order, their names and the open one", () => {
    ...
    assert.deepEqual(r.pages.map((p) => p.name), ["the green blanket", "the blue cardigan"]);
    assert.equal(r.open, "the blue cardigan");
    assert.deepEqual(r.pages[1].fields, { patternGauge: "18", patternRowGauge: "24", sizesText: "32, 36, 40", yardsText: "900, 1000, 1100", bust: "38", myGauge: "21", myRowGauge: "26", perSkein: "220", skeins: "6" });
  ...
  test("an open name that matches no page falls back to the first", () => {
    assert.equal(readNotebook(saved({ open: "the red hat", pages: [green, blue] })).open, "the green blanket");
  ...
  test("an easeIdx of 7, furlongs and macramé are refused", () => {
    const r = openPage(readNotebook(saved({ units: "furlongs", craft: "macrame", easeIdx: 7, bust: "38" })));
  ...
  test("names are trimmed and capped, and a missing name is empty", () => {
    ...
    assert.equal(readNotebook(saved({ pages: [{ ...blue, name: "  padded  " }] })).pages[0].name, "padded");
  });
...
describe("openPage and personalOnly", () => {
  test("the open page, else the first, else nothing", () => {
    assert.equal(openPage(readNotebook(saved({ open: "the blue cardigan", pages: [green, blue] }))).name, "the blue cardigan");
  ...
  test("personalOnly keeps the knitter's fields and drops the pattern's", () => {
    ...
    assert.deepEqual(p.fields, { bust: "38", myGauge: "21", myRowGauge: "26", perSkein: "220", skeins: "6" });
    assert.equal(p.units, "in");
...
describe("notebookInUnits: the page follows the link's units", () => {
  const inches = { name: "", units: "in", craft: "knit", easeIdx: 2, fields: { bust: "38", myGauge: "21", myRowGauge: "26", perSkein: "220", skeins: "6" } };
  ...
  test("a whole page converts its pattern too, lists and all", () => {
    const whole = { ...inches, fields: { ...inches.fields, patternGauge: "18", patternRowGauge: "24", sizesText: "32, 36", yardsText: "900, 1000" } };
