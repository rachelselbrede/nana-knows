...
export function MathNote({ t }) {
  return (
    ...
    >
      ...
      <div
        className="mt-3 text-sm leading-relaxed flex flex-col gap-2"
        style={{ fontFamily: SANS, color: C.ink }}
      >
        <p>
          <strong>{t("math.labels.size")}</strong> {t("math.size")}
        </p>
        <p>
          <strong>{t("math.labels.yarn")}</strong> {t("math.yarn")}
        </p>
        <p>
          <strong>{t("math.labels.tension")}</strong> {t("math.tension")}
        </p>
        <p>
          <strong>{t("math.labels.length")}</strong> {t("math.length")}
        </p>
      </div>
    </details>
  );
